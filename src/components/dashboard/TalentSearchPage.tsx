...
import { Search, Zap, AlertCircle, MapPin, GraduationCap, Award, Languages, Filter, SortDesc, Users, FileText, CheckCircle2 } from 'lucide-react'
...
export function TalentSearchPage() {
  ...
  useEffect(() => {
    const scoreTypeData = MATCH_SCORE_TYPES[matchScoreType as keyof typeof MATCH_SCORE_TYPES]
  ...
  const handleSearch = async () => {
    ...
    try {
      // Simulate longer processing time for higher accuracy
      const scoreTypeData = MATCH_SCORE_TYPES[matchScoreType as keyof typeof MATCH_SCORE_TYPES]
      const processingTime = matchScoreType === 'quick' ? 2000 : 
                            matchScoreType === 'balanced' ? 3000 :
                            matchScoreType === 'detailed' ? 4000 : 5000
      
      await new Promise(resolve => setTimeout(resolve, processingTime))
      
      // Use demo data for results
      let searchResults = [...hrTestData]
      
      // Filter based on job title and skills
      if (jobTitle.trim()) {
        searchResults = searchResults.filter(candidate => 
          candidate['Current Position'].toLowerCase().includes(jobTitle.toLowerCase()) ||
          candidate['Full Name'].toLowerCase().includes(jobTitle.toLowerCase())
        )
      }
      
      if (skillsRequired.trim()) {
        const skillKeywords = skillsRequired.split(',').map(s => s.trim().toLowerCase())
        searchResults = searchResults.filter(candidate =>
          skillKeywords.some(skill => 
            candidate['Skills Match'].toLowerCase().includes(skill) ||
            candidate['Current Position'].toLowerCase().includes(skill)
          )
        )
      }
      ...
      if (educationLevel) {
        // Simple education filtering based on the selected level
        searchResults = searchResults.filter(candidate => {
          const education = candidate['Education Match'] || ''
          return education.toLowerCase().includes(educationLevel.toLowerCase())
        })
      }
      
      // Adjust match scores based on selected type
      searchResults = searchResults.map(candidate => ({
        ...candidate,
        'Match Score (0-100)': Math.min(100, Math.max(30, 
          candidate['Match Score (0-100)'] * (scoreTypeData.percentage / 60) // Base adjustment
        ))
      }))
      
      // Limit results to requested number of candidates
      searchResults = searchResults.slice(0, numberOfCandidates)
      ...
      setShowResults(true)
      toast.success(`${t('services.talent_search.candidates_found')}: ${searchResults.length}!`)
    } catch (error: any) {
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center bg-slate-800/50 backdrop-blur-sm rounded-xl p-4 shadow-lg border border-slate-700">
            ...
            <span className="text-white font-medium">
              {t('current_balance')}: {balance?.toLocaleString() || 0} {t('credit.balance')}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-8 shadow-lg border border-slate-700 mb-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#10172A] rounded-xl p-6 border border-[#334155]">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[#CBD5E1] mb-2">
                  {t('services.talent_search.job_description')} ({t('form.optional')})
                </label>
                <textarea
          ...
          <div className="bg-[#10172A] rounded-xl p-6 border border-[#334155]">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#CBD5E1] mb-2">
                    {t('services.talent_search.certifications')} ({t('form.optional')})
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-[#CBD5E1] mb-2">
                    {t('services.talent_search.languages')} ({t('form.optional')})
                  </label>
                  <input
                    type="text"
                    value={languages}
              ...
              <div>
                <label className="block text-sm font-medium text-[#CBD5E1] mb-2">
                  {t('services.talent_search.education_level')} ({t('form.optional')})
                </label>
                <select
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNumberOfCandidates(parseInt(e.target.value))}
                  className="w-full px-4 py-3 bg-[#1E293B] border border-[#334155] rounded-lg text-[#F8FAFC] focus:outline-none focus:ring-2 focus:ring-[#38BDF8] focus:border-[#38BDF8]"
                >
                  <option value={5}>5 {t('services.talent_search.candidates')}</option>
                  <option value={10}>10 {t('services.talent_search.candidates')}</option>
                  <option value={15}>15 {t('services.talent_search.candidates')}</option>
                  <option value={20}>20 {t('services.talent_search.candidates')}</option>
                </select>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    <div className="text-center">
                      ...
                      <h4 className="font-semibold text-[#F8FAFC] mb-1">{t('services.talent_search.quick_search')}</h4>
                      <div className="text-sm text-[#CBD5E1] mb-2">50%</div>
                      <div className="text-xs text-[#94A3B8] mb-2">{t('services.talent_search.quick_description')}</div>
                      <div className="font-bold text-[#22C55E]">10 {t('credit.balance')}</div>
                      <div className="text-xs text-[#94A3B8]">{t('services.talent_search.per_candidate')}</div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-1">🔵</div>
                      <h4 className="font-semibold text-[#F8FAFC] mb-1">{t('services.talent_search.balanced_search')}</h4>
                      <div className="text-sm text-[#CBD5E1] mb-2">60%</div>
                      <div className="text-xs text-[#94A3B8] mb-2">{t('services.talent_search.balanced_description')}</div>
                      <div className="font-bold text-[#38BDF8]">15 {t('credit.balance')}</div>
                      <div className="text-xs text-[#94A3B8]">{t('services.talent_search.per_candidate')}</div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-1">🟡</div>
                      <h4 className="font-semibold text-[#F8FAFC] mb-1">{t('services.talent_search.detailed_search')}</h4>
                      <div className="text-sm text-[#CBD5E1] mb-2">70%</div>
                      <div className="text-xs text-[#94A3B8] mb-2">{t('services.talent_search.detailed_description')}</div>
                      <div className="font-bold text-[#EAB308]">20 {t('credit.balance')}</div>
                      <div className="text-xs text-[#94A3B8]">{t('services.talent_search.per_candidate')}</div>
                    </div>
                  </div>
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-1">🔴</div>
                      <h4 className="font-semibold text-[#F8FAFC] mb-1">{t('services.talent_search.comprehensive_search')}</h4>
                      <div className="text-sm text-[#CBD5E1] mb-2">80%</div>
                      <div className="text-xs text-[#94A3B8] mb-2">{t('services.talent_search.comprehensive_description')}</div>
                      <div className="font-bold text-[#F43F5E]">25 {t('credit.balance')}</div>
                      <div className="text-xs text-[#94A3B8]">{t('services.talent_search.per_candidate')}</div>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-[#1E293B] border border-[#334155] rounded-xl p-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-[#CBD5E1]">
                ...
                <span className="font-medium">{MATCH_SCORE_TYPES[matchScoreType as keyof typeof MATCH_SCORE_TYPES].total} {t('credit.balance')}</span>
              </div>
              <div className="border-t border-[#334155] pt-2 mt-2">
                <div className="flex justify-between items-center text-lg font-bold text-[#38BDF8]">
                  ...
                  <span>{totalCost.toLocaleString()} {t('credit.balance')}</span>
                </div>
              </div>
                ...
                <div className="mt-3 p-3 bg-[#10172A] border border-[#F43F5E] rounded-lg">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-[#F43F5E]">
                      {t('credit.insufficient')} - {t('current_balance')}: {balance?.toLocaleString() || 0} {t('credit.balance')}
                    </span>
                  </div>
                </div>
              )}
        ...
        <div className="mt-8">
          <button
            ...
            disabled={searching || balance < totalCost || !jobTitle.trim() || !skillsRequired.trim()}
            className={`w-full py-4 px-6 rounded-xl font-semibold text-white transition-all duration-200 ${
              searching || balance < totalCost || !jobTitle.trim() || !skillsRequired.trim()
          ...
          >
            {searching ? (
              ...
              `${t('services.talent_search.start_search')} (${totalCost.toLocaleString()} ${t('credit.balance')})`
            )}
          </button>
        </div>
      </div>
        ...
        <div className="mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-[#22C55E]/10 border border-[#22C55E]/30 rounded-lg p-3 text-center">
                <div className="text-[#22C55E] text-2xl font-bold">
                  {getFilteredAndSortedResults().filter(c => c['Match Score (0-100)'] >= 80).length}
                </div>
              ...
              <div className="bg-[#EAB308]/10 border border-[#EAB308]/30 rounded-lg p-3 text-center">
                <div className="text-[#EAB308] text-2xl font-bold">
                  {getFilteredAndSortedResults().filter(c => c['Match Score (0-100)'] >= 60 && c['Match Score (0-100)'] < 80).length}
                </div>
              ...
              <div className="bg-[#F43F5E]/10 border border-[#F43F5E]/30 rounded-lg p-3 text-center">
                <div className="text-[#F43F5E] text-2xl font-bold">
                  {getFilteredAndSortedResults().filter(c => c['Match Score (0-100)'] < 60).length}
                </div>
          ...
          {getFilteredAndSortedResults().length > 0 ? (
            ...
            <div className="bg-[#1E293B] backdrop-blur-sm rounded-xl p-8 border border-[#334155] text-center">
              ...
              <h3 className="text-lg font-medium text-[#F8FAFC] mb-2">لا توجد نتائج</h3>
              <p className="text-gray-400">لم يتم العثور على مرشحين يطابقون معايير البحث المحددة.</p>
            </div>
          )}
        </div>
      )}
