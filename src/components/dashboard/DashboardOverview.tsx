...
export function DashboardOverview() {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div className={`flex flex-col md:flex-row md:items-center md:justify-between ${isRTL() ? 'md:flex-row-reverse' : ''}`}>
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">
              {t('message.welcome')}, {profile?.full_name || t('message.welcome')}
            </h1>
            ...
            >
              <Link to="/pricing">
                <Button className="group">
                  ...
                  {t('credit.buy_more')}
                  <ArrowRight className={`h-4 w-4 group-hover:translate-x-1 transition-transform ${isRTL() ? 'mr-2 group-hover:-translate-x-1' : 'ml-2'}`} />
                </Button>
              </Link>
            </motion.div>
          )}
          ...
          {quickActions.map((action, index) => (
            ...
            >
              <Card className="group hover:transform hover:scale-105 transition-all duration-300 h-full">
                <div className="p-6">
                  <div className={`w-12 h-12 rounded-lg bg-gradient-to-r ${action.gradient} p-3 mb-4 group-hover:scale-110 transition-transform duration-300`}>
                    <action.icon className="h-6 w-6 text-white" />
                  </div>
                  <h3 className="text-lg font-semibold text-white mb-2">{action.title}</h3>
                  <p className="text-gray-400 text-sm mb-4">{action.description}</p>
                  <div className={`flex items-center justify-between ${isRTL() ? 'flex-row-reverse' : ''}`}>
                    <span className="text-xs text-cyan-400 font-medium">{action.cost}</span>
                    <Link to={action.href}>
                      <Button size="sm" variant="secondary" className="group">
                        {isRTL() ? 'ابدأ الآن' : 'Start Now'}
                        <ArrowRight className={`h-3 w-3 group-hover:translate-x-1 transition-transform ${isRTL() ? 'mr-2 group-hover:-translate-x-1' : 'ml-2'}`} />
                      </Button>
                    </Link>
                  </div>
                </div>
        ...
        <div className={`flex items-center justify-between mb-6 ${isRTL() ? 'flex-row-reverse' : ''}`}>
          ...
          <Link to="/dashboard/credit-history">
            <Button variant="text" size="sm" className="group">
              ...
              <History className={`h-4 w-4 group-hover:translate-x-1 transition-transform ${isRTL() ? 'mr-2 group-hover:-translate-x-1' : 'ml-2'}`} />
            </Button>
          </Link>
        </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-400 mb-4">
                  {isRTL() ? 'ابدأ استخدام النظام لرؤية أنشطتك هنا' : 'Start using the system to see your activities here'}
                </p>
              ...
              <div className="space-y-4">
                {recentActivity.map((activity, index) => (
                  <div key={activity.id || index} className={`flex items-center justify-between p-4 bg-slate-700/30 rounded-lg ${isRTL() ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`text-red-400 font-semibold ${isRTL() ? 'text-left' : 'text-right'}`}>
