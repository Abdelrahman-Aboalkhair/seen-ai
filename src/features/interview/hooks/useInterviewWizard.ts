...
import { InterviewData, TestType, Question, Candidate, TEST_TYPES, DURATION_OPTIONS } from "../types";
...
export const useInterviewWizard = () => {
  ...
  const calculateInterviewDetails = useCallback((selectedTypes: TestType[], duration: number) => {
    ...
    actualTestTypes.forEach(testType => {
      const questionsPerType = Math.floor(durationOption.questionCount / actualTestTypes.length);
  ...
  const updateInterviewData = useCallback((updates: Partial<InterviewData>) => {
  ...
  const toggleTestType = useCallback((testType: TestType) => {
    setInterviewData(prev => {
      const isSelected = prev.selectedTestTypes.some(t => t.id === testType.id);
      let newSelectedTypes: TestType[];

      if (isSelected) {
        newSelectedTypes = prev.selectedTestTypes.filter(t => t.id !== testType.id);
      } else {
        // Check if we can add more test types based on duration
        const durationOption = DURATION_OPTIONS.find(opt => opt.value === prev.durationMinutes);
        if (durationOption && prev.selectedTestTypes.length >= durationOption.maxTestTypes) {
          toast.error(`يمكنك اختيار ${durationOption.maxTestTypes} أنواع اختبار كحد أقصى لمدة ${prev.durationMinutes} دقيقة`);
          return prev;
        }
        newSelectedTypes = [...prev.selectedTestTypes, testType];
      }

      const { totalQuestions, creditsUsed } = calculateInterviewDetails(newSelectedTypes, prev.durationMinutes);
  ...
  const generateQuestions = useCallback(async () => {
    ...
    if (balance < interviewData.creditsUsed) {
      toast.error(`رصيد الكريدت غير كافي. المطلوب: ${interviewData.creditsUsed}، المتوفر: ${balance}`);
      return;
    }
    ...
    try {
      ...
      await deductCredits(interviewData.creditsUsed, `إنشاء أسئلة مقابلة - ${interviewData.jobTitle}`);
      ...
      for (const testType of interviewData.selectedTestTypes) {
        const questionsPerType = Math.floor(interviewData.totalQuestions / interviewData.selectedTestTypes.length);
        ...
        if (error) {
          throw new Error(`فشل في إنشاء أسئلة ${testType.label}: ${error.message}`);
        }

        if (data?.questions) {
          const testQuestions: Question[] = data.questions.map((q: any, index: number) => ({
  ...
  const fetchCandidates = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('id, full_name, email, resume_url')
  ...
  const addCandidatesToInterview = useCallback(async (selectedCandidateIds: string[]) => {
    if (!interviewData.id) {
      ...
      for (const candidate of interviewCandidates) {
        ...
        const { data: session, error: sessionError } = await supabase
          ...
          .insert({
            ...
            session_token: btoa(Math.random().toString(36) + Date.now().toString(36)),
            expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() // 7 days
          })
          .select()
          .single();
        ...
        if (emailError) {
          console.warn(`Failed to send email to ${candidate.email}:`, emailError);
        }
      }
