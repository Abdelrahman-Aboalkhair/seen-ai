...
export class CVAnalysisService implements ICVAnalysisService {
  ...
  async analyzeCV(request: CVAnalysisRequest): Promise<CVAnalysisResult> {
    ...
    if (!this.validateRequest(request)) {
      console.log("❌ [CV Service] Request validation failed");
    ...
    try {
      console.log("⚙️ [CV Service] Calling repository to analyze CV...");
      // Analyze CV using repository
      const result = await this.repository.analyzeCV(enrichedRequest);

      console.log("📊 [CV Service] Repository returned result:", {
        hasResult: !!result,
        score: result?.score,
      ...
      if (!result) {
        console.log("❌ [CV Service] No result generated by repository");
