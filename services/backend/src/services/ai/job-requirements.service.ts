...
export class JobRequirementsService extends BaseAIService {
  ...
  // Generate job requirements with caching
  async generateJobRequirements(request: JobRequirementsRequest): Promise<JobRequirementsResult> {
    ...
    try {
      ...
      const result = await this.withRetry(
        () => this.generateJobDescription(request),
  ...
  private async generateJobDescription(request: JobRequirementsRequest): Promise<JobRequirementsResult> {
    ...
    return this.parseJsonResponse<JobRequirementsResult>(content, 'job_requirements');
  }

  // Generate job requirements for multiple positions
  async generateMultipleJobRequirements(
    requests: JobRequirementsRequest[]
  ): Promise<JobRequirementsResult[]> {
    const results = await Promise.all(
      requests.map(request => this.generateJobRequirements(request))
    );

    return results;
  }

  // Generate job requirements with specific focus
  async generateFocusedJobRequirements(
    request: JobRequirementsRequest,
    focus: 'technical' | 'leadership' | 'creative' | 'sales'
  ): Promise<JobRequirementsResult> {
    const focusPrompts = {
    ...
    return this.parseJsonResponse<JobRequirementsResult>(content, 'focused_job_requirements');
  }

  // Update existing job requirements
  async updateJobRequirements(
    existingRequirements: JobRequirementsResult,
    updates: Partial<JobRequirementsRequest>
  ): Promise<JobRequirementsResult> {
    ...
    return this.parseJsonResponse<JobRequirementsResult>(content, 'updated_job_requirements');
  }
}
