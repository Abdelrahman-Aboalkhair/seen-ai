...
export class CacheService {
  ...
  async setCVAnalysis(cvText: string, jobRequirements: string, userId: string, result: any) {
    const key = 'cv_analysis';
    const data = { cvText, jobRequirements, userId };
    const cacheKey = this.generateKey(key, data);
  ...
  async setQuestions(jobTitle: string, skills: string[], count: number, result: any) {
    const key = 'questions';
    const data = { jobTitle, skills: skills.sort(), count };
    const cacheKey = this.generateKey(key, data);
  ...
  async getJobRequirements(jobInfo: any) {
  ...
  async setJobRequirements(jobInfo: any, result: any) {
    const key = 'job_requirements';
    const cacheKey = this.generateKey(key, jobInfo);
  ...
  async incrementRateLimit(userId: string, endpoint: string, windowMs: number = 900000): Promise<number> {
    const key = `rate_limit:${userId}:${endpoint}`;
  ...
  async multiSet<T>(keyValuePairs: Record<string, T>, options: CacheOptions = {}): Promise<boolean> {
    try {
      ...
      logError(error as Error, { operation: 'cache_multi_set', keys: Object.keys(keyValuePairs) });
      return false;
    }
  }
