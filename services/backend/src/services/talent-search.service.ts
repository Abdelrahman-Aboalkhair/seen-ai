...
export class TalentSearchService implements ITalentSearchService {
  ...
  async searchTalent(
    ...
    try {
      ...
      // Note: Match scores are now calculated by N8N, not locally
      console.log(
        "Service: Search completed successfully, found",
  ...
  async advancedSearch(
    ...
    console.log("Service: Processing advanced talent search:", {
      criteria,
      filters,
    });
    ...
    try {
      ...
      const result = await this.repository.advancedSearch(
        enrichedCriteria,
        filters
      );
      ...
      // Note: Match scores are now calculated by N8N, not locally
      console.log(
        "Service: Advanced search completed successfully, found",
        result.profiles.length,
        "profiles"
      );
  ...
  private validateAdvancedFilters(filters: AdvancedSearchFilters): void {
    ...
    if (
      filters.companySize &&
      !["startup", "mid-size", "enterprise"].includes(filters.companySize)
    ) {
    ...
    if (
      filters.relocation !== undefined &&
      typeof filters.relocation !== "boolean"
    ) {
      throw new Error("Invalid relocation filter");
    }
  }

  private enrichSearchCriteria(
    criteria: TalentSearchCriteria
  ): TalentSearchCriteria {
    ...
    const scoredProfiles = result.profiles.map((profile) => {
      ...
      if (criteria.experience) {
        ...
        if (profile.experience >= min && profile.experience <= max) {
          ...
          score += Math.max(
            0,
            25 - Math.abs(profile.experience - (min + max) / 2) * 2
          );
        }
      }
      ...
      if (
        criteria.availability &&
        profile.availability === criteria.availability
      ) {
        score += 5;
      }
